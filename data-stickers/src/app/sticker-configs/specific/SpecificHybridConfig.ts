...
export class SpecificHybridConfig extends StickerConfig {
	plainSvgURL:string;
	...
	animation_options(el, svg, param, options) {
		if(this.stickerType == "chartjunk") {
			...
			if(param == 'fill') {
				var gsap_animation = tl.to("#fill-mask", {duration:0, xPercent:0})
				.to("#fill-mask", {duration:1, xPercent:percent, stagger:0.4})
					.to(svg, 0.8, {
					scaleX: 1.05,
					scaleY: 1.05,
					// @ts-ignore
			...
			tl.to("#fill-mask", {duration:0, xPercent:percent}).play();
		}
